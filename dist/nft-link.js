...
exports.parseNftLink = parseNftLink;
...
function normalizeChain(segment) {
...
function parseNftLink(input) {
    const raw = input.trim().replace(/\/+$/, "");
    if (!raw)
        throw new Error("No NFT link provided");
    ...
    if (!/^[a-zA-Z0-9._-]+$/.test(raw)) {
        throw new Error(`Could not read "${input}" as an OpenSea link, collection slug, or contract address`);
    }
...
function parseUrl(raw) {
    ...
    if (looseAddr)
        return { kind: "address", value: looseAddr };
    throw new Error(`No collection slug or contract address found in "${raw}" — paste the OpenSea collection or item link`);
}
//# sourceMappingURL=nft-link.js.map